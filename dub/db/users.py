...
def _mongo_to_user(*, db_user: models.User) -> User:
    return User(
        uuid=db_user.uuid,
        login=db_user.login,
        email=db_user.email,
...
class Users(BaseUserRepo):
    ...
    def get_user(**kw: Dict[str, Union[str, UUID]]) -> Union[ErrorResponse, User]:
        query_set = models.User.objects.none()
        ...
        for key in kw:
            if kw[key]:
                ...
                if isinstance(value, UUID):
                    value = str(value)
                query_set = models.User.objects(Q(**{f"{key}__iexact": value}) | query_set._query_obj)
    ...
    def check_password(*, clean: str, uuid: str) -> Union[ErrorResponse, StatusResponse]:
        try:
            db_user = models.User.objects(uuid=uuid).first()
    ...
    def save_user(*, user: User) -> Union[ErrorResponse, User]:
        db_user = models.User.objects(uuid=str(user.uuid)).first()
        if not user:
